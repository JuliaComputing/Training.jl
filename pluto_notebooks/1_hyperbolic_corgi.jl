...
begin
	...
	xy_to_ij((x,y),img::AbstractMatrix) = xy_to_ij((x,y),size(img))

	function xy_to_ij((x,y),(rows,cols)) 
		m = max(cols, rows)	
		φ = translate(rows/2, cols/2) ∘ swap ∘ flipy ∘ scale(m/2)
		i,j = floor.(Int, φ((x,y)))
	end

	function ij_to_xy( (i,j), m)
		φ =  scale(2/m) ∘ flipy ∘ swap ∘ translate(-m/2,-m/2)
		φ((i,j))
	end
...
begin
	 ...
	 function getpixel(img,i::Int,j::Int)
			#  grab image color or  default
			rows, cols = size(img)

			if 1 ≤ i ≤ rows && 1 ≤ j ≤ cols 
				img[i, j]
			else
				black(img[1,1]) # or default to black
			end
		  end
end
...
function transform_image(T,pic,pixels=800)
	...
	[			
	  begin		 
			 x, y = ij_to_xy((i,j), pixels)
			 X, Y = T((x,y))
			 i, j = xy_to_ij((X,Y),pic)
			 getpixel(pic,i,j)
	   end	 
...
begin
	...
	# red circle
	t = (0:.01:1).*(2π)
	plot!( 400 .+310 .*cos.(t), 400 .+ 310 .* sin.(t), color=:red, lw=10)
	
	# green hyperbolas
	t= -10:.01:10
	s = 80
	plot!( 400 .+s .*cosh.(t), 400 .+ s .* sinh.(t), color=:green, lw=10)
	plot!( 400 .-s .*cosh.(t), 400 .+ s .* sinh.(t), color=:green, lw=10)
	plot!( 400 .+s .*sinh.(t), 400 .+ s .* cosh.(t), color=:green, lw=10)
	plot!( 400 .-s .*sinh.(t), 400 .- s .* cosh.(t), color=:green, lw=10, legend=false)
end
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "ea492309e7e493f188de604ff3358f4c1da5fe38"
